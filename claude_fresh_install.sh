...
install_packages() {
    ...
    case $pkg_manager in
        "apt")
            sudo apt update
            sudo apt install -y git vim tmux curl zsh python3 python3-pip nodejs npm flatpak
            ;;
        "dnf"|"yum")
            sudo $pkg_manager update -y
            sudo $pkg_manager install -y git vim tmux curl zsh python3 python3-pip nodejs npm flatpak
            ;;
        *)
            echo "Unsupported package manager"
            exit 1
...
echo "yes i! configuration complete! restart for maximum freshness."
